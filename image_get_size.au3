...
; Web:              www.autoitscript.com/fileman/users/Lazycat/udfs.html
; Version:          1.1.00
; Date:             15.03.2005
;
;===============================================================================
...
Func _ImageGetSizeTIF($sFile, $nByteOrder)
    Local $anSize[2]
    Local $nTagsOffset = _Dec(_FileReadAtOffsetHEX($sFile, 5, 4), $nByteOrder) + 1
    Local $nFieldCount = _Dec(_FileReadAtOffsetHEX($sFile, $nTagsOffset, 2), $nByteOrder)
    For $i = 0 To $nFieldCount - 1
        Local $sField = _FileReadAtOffsetHEX($sFile, $nTagsOffset + 2 + 12 * $i, 12)
        Local $sTag = StringLeft($sField, 4)
