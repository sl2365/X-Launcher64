...
set "TEST_ROOT=%KIT_ROOT%Stage6K_Full_Test_Java_Transaction"
set "TEST_EXE=%TEST_ROOT%\Launcher\X-Launcher_x64.exe"
set "TEST_INI=%TEST_ROOT%\Stage6K_Full_Test_Java_Transaction.ini"
set "CONFIGURED_ROOT=%TEST_ROOT%\ConfiguredRoot"
set "RESULTS=%KIT_ROOT%Stage6K_Full_Test_Java_Transaction_Results.txt"
set "REPORT="
set "WORKSPACE="
set "SELFTEST_REG="
set "VIEW_REG="
...
cd /d "%PROJECT_ROOT%"
title X-Launcher Stage 6K Full Test Java Transaction

echo ============================================================
echo X-LAUNCHER STAGE 6K - FULL TEST JAVA TRANSACTION
echo ============================================================
echo.
...
if exist "%TEST_ROOT%" (
    echo ERROR: The previous Stage 6K test folder could not be removed.
    goto EARLY_FAIL
)
...
>"%TEST_INI%" (
    echo [Setup]
    echo AppName=Stage6KConfiguredTargetsMustNotRun
...
reg delete "HKCU\Software\XLauncher_Test\Stage6K_Full" /f >nul 2>&1
reg add "HKCU\Software\XLauncher_Test\Stage6K_Full" /v State /t REG_SZ /d HOST /f >nul 2>&1

>"%RESULTS%" (
    echo X-LAUNCHER STAGE 6K FULL TEST JAVA TRANSACTION RESULTS
    echo ========================================================
    echo.
)

echo A Full X-Launcher Test confirmation will appear.
echo Click Yes to run the isolated built-in Java transaction tests.
echo.
echo Every fake runtime and package remains beneath the Full Test Temp workspace.
echo No network connection Java process or real Java installation is used.
echo The configured payload environment URL and operations must not run.
...
call :CHECK_TEXT "Java transaction fixtures passed workspace boundary" "[PASS] [Java Transaction] Every Java transaction fixture passed the isolated workspace boundary check" "!REPORT!"
call :CHECK_TEXT "Java transaction fixtures were created" "[PASS] [Java Transaction] Isolated runtime package staging backup and rollback fixtures were created" "!REPORT!"
call :CHECK_TEXT "JavaURL schemes were validated without network access" "[PASS] [Java Transaction] JavaURL accepted only direct HTTP and HTTPS values without network access" "!REPORT!"
call :CHECK_TEXT "Local setup packages were recognized without execution" "[PASS] [Java Transaction] Local ZIP and MZ legacy setup packages were recognized without execution" "!REPORT!"
call :CHECK_TEXT "Direct and wrapped runtime layouts were accepted" "[PASS] [Java Transaction] Direct and single-wrapper staged runtime layouts were accepted" "!REPORT!"
call :CHECK_TEXT "Ambiguous and incomplete layouts were rejected" "[PASS] [Java Transaction] Ambiguous and incomplete staged runtime layouts were rejected" "!REPORT!"
call :CHECK_TEXT "Failed extraction cleaned staging before live change" "[PASS] [Java Transaction] Failed package extraction removed staging before the live runtime was touched" "!REPORT!"
call :CHECK_TEXT "Backup path validation allowed only a direct child" "[PASS] [Java Transaction] Backup path validation allowed only one direct child of the live runtime" "!REPORT!"
call :CHECK_TEXT "Complete old runtime was backed up" "[PASS] [Java Transaction] Complete runtime content was backed up before live replacement" "!REPORT!"
call :CHECK_TEXT "Setup and staging data stayed outside backup" "[PASS] [Java Transaction] Setup and staging data stayed outside the portable runtime backup" "!REPORT!"
call :CHECK_TEXT "Prepared complete runtime was installed" "[PASS] [Java Transaction] Prepared runtime installation copied the complete new runtime into place" "!REPORT!"
call :CHECK_TEXT "Cancellation stopped before destination mutation" "[PASS] [Java Transaction] Cancellation stopped prepared installation before destination mutation" "!REPORT!"
call :CHECK_TEXT "Rollback restored complete original runtime" "[PASS] [Java Transaction] Rollback removed partial content and restored the complete original runtime" "!REPORT!"
call :CHECK_TEXT "Java transaction process state was restored" "[PASS] [Java Transaction] Java transaction working directory and cancellation state were restored" "!REPORT!"
...
reg query "HKCU\Software\XLauncher_Test\Stage6K_Full" /v State 2>nul | find /i "HOST" >nul
if not errorlevel 1 (
...
if not "!FAIL_COUNT!"=="0" (
    echo STAGE 6K FULL TEST JAVA TRANSACTION: FAIL
    >>"%RESULTS%" echo.
    >>"%RESULTS%" echo Overall: FAIL
    echo.
    echo Please provide Stage6K_Full_Test_Java_Transaction_Results.txt and
    echo Full_Test_Report.log.
    goto FINISH_FAIL
)

echo STAGE 6K FULL TEST JAVA TRANSACTION: PASS
>>"%RESULTS%" echo.
...
:EARLY_FAIL
echo.
echo STAGE 6K FULL TEST JAVA TRANSACTION: NOT RUN
echo.
pause
exit /b 1
