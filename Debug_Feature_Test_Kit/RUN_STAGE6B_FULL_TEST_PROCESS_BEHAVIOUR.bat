...
set "TEST_ROOT=%KIT_ROOT%Stage6B_Full_Test_Process_Behaviour"
set "TEST_EXE=%TEST_ROOT%\Launcher\X-Launcher_x64.exe"
set "TEST_INI=%TEST_ROOT%\Stage6B_Full_Test_Process_Behaviour.ini"
set "CONFIGURED_ROOT=%TEST_ROOT%\ConfiguredRoot"
set "RESULTS=%KIT_ROOT%Stage6B_Full_Test_Process_Behaviour_Results.txt"
set "REPORT="
set "WORKSPACE="
set "SELFTEST_REG="
set "WORKSPACE_LINE="
...
cd /d "%PROJECT_ROOT%"
title X-Launcher Stage 6B Full Test Process Behaviour

echo ============================================================
echo X-LAUNCHER STAGE 6B - FULL TEST PROCESS BEHAVIOUR
echo ============================================================
echo.
...
if exist "%TEST_ROOT%" (
    echo ERROR: The previous Stage 6B test folder could not be removed.
    goto EARLY_FAIL
)
...
>"%TEST_INI%" (
    echo [Setup]
    echo AppName=Stage6BConfiguredTargetsMustNotRun
...
reg delete "HKCU\Software\XLauncher_Test\Stage6B_Full" /f >nul 2>&1
reg add "HKCU\Software\XLauncher_Test\Stage6B_Full" /v State /t REG_SZ /d HOST /f >nul 2>&1

>"%RESULTS%" (
    echo X-LAUNCHER STAGE 6B FULL TEST PROCESS BEHAVIOUR RESULTS
    echo ========================================================
    echo.
)

echo A Full X-Launcher Test confirmation will appear.
echo Click Yes to run the isolated built-in process tests.
echo.
echo Two hidden private helper copies will briefly run together.
...
call :CHECK_TEXT "Missing executable launch failure was detected" "[PASS] [Process] Missing executable launch failure was detected:" "!REPORT!"
call :CHECK_TEXT "Two private helper instances ran concurrently" "[PASS] [Process] Two isolated self-helper instances ran concurrently:" "!REPORT!"
call :CHECK_TEXT "Both private helper instances closed normally" "[PASS] [Process] Both isolated self-helper instances closed normally" "!REPORT!"
call :CHECK_TEXT "Stage 6A helper success contract still passes" "[PASS] [Self Helper] Private helper completed with success exit code: exit=0; error=0" "!REPORT!"
call :CHECK_TEXT "Stage 6A controlled nonzero exit still passes" "[PASS] [Process] Controlled nonzero helper exit code was observed: exit=23; error=0" "!REPORT!"
...
reg query "HKCU\Software\XLauncher_Test\Stage6B_Full" /v State 2>nul | find /i "HOST" >nul
if not errorlevel 1 (
...
if not "!FAIL_COUNT!"=="0" (
    echo STAGE 6B FULL TEST PROCESS BEHAVIOUR: FAIL
    >>"%RESULTS%" echo.
    >>"%RESULTS%" echo Overall: FAIL
    echo.
    echo Please provide Stage6B_Full_Test_Process_Behaviour_Results.txt and
    echo Full_Test_Report.log.
    goto FINISH_FAIL
)

echo STAGE 6B FULL TEST PROCESS BEHAVIOUR: PASS
>>"%RESULTS%" echo.
...
:EARLY_FAIL
echo.
echo STAGE 6B FULL TEST PROCESS BEHAVIOUR: NOT RUN
echo.
pause
exit /b 1
